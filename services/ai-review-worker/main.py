...
def process_event(event: dict, trace_id: str = "unknown"):
    ...
    log_event("claude_call", trace_id, pr_number=pr_number, duration_ms=round((time.perf_counter() - t_ai) * 1000, 2))
    ...
    log_event("ai_review", trace_id, pr_number=pr_number, duration_ms=round((time.perf_counter() - t0) * 1000, 2))
