...
from kafka_reliability import process_with_retry
...
# ── DLQ / retry tests (shared kafka_reliability module) ─────────────────────

@patch("kafka_reliability.send_to_dlq")
def test_process_with_retry_success(mock_dlq):
    """Handler 成功 → 不发 DLQ，返回 True（可以安全 commit）"""
    handler = MagicMock()
    result = process_with_retry("result-service", "localhost:9092", handler, AI_DATA, "{}")
...
def test_process_with_retry_permanent_failure(mock_dlq):
    ...
    assert mock_dlq.call_args[0][4] == "PermanentFailure"
    assert result is True


@patch("kafka_reliability.time.sleep")
@patch("kafka_reliability.send_to_dlq")
def test_process_with_retry_exhausted(mock_dlq, mock_sleep):
    ...
    mock_dlq.assert_called_once()
    assert mock_dlq.call_args[0][4] == "TemporaryFailure"
    assert result is True
